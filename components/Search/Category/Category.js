...
const Category = () => {
    ...
    // if (error) toast.error("Ошибка категорий")
    console.log(data)
                    ...
                    >
                        <div className={styles.categoryHead}>
                            <div className={styles.categoryBody}>
                                <img width={"24px"} height={"24px"} src={item.image || ""} alt=""/>
                                {item.name}
                            </div>
                            <img src="/icons/categoryArrow.png" alt=""/>
                        </div>
                        {item.children.map(child => {
                            return <SubCategories key={child.id} subCategory={child}/>
                        })}
                    </li>
                })}
            </ul>
